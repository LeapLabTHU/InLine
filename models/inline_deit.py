...
class InLineBlock(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, window=14):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = InLineAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                                    window=window)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            InLineBlock(
                ...
                window=img_size // patch_size) for i in range(depth)])
...
@register_model
def inline_deit_tiny(pretrained=False, **kwargs):
...
@register_model
def inline_deit_small(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=320, depth=12, num_heads=10, **kwargs)
    model = _create_vision_transformer('deit_tiny_patch16_224', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def inline_deit_base(pretrained=False, **kwargs):
