...
class InLineAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0., **kwargs):
        super().__init__()
        self.dim = dim
        self.dim_out = dim_out or dim
        self.resolution = resolution
    ...
    def forward(self, qkv, x_mean):
        ...
        res_weight = self.residual(x_mean.unsqueeze(dim=-1)).reshape(b * c, 1, 3, 3)
...
class CSWinBlock(nn.Module):
    def __init__(self, dim, reso, num_heads,
        ...
        attn = InLineAttention if attn_type == 'I' else LePEAttention
        if last_stage:
    ...
    def forward(self, x):
        ...
        if self.branch_num == 2:
            x1 = self.attns[0](qkv[:, :, :, :C // 2], x.mean(dim=1)[:, :C // 2])
            x2 = self.attns[1](qkv[:, :, :, C // 2:], x.mean(dim=1)[:, C // 2:])
            attened_x = torch.cat([x1, x2], dim=2)
        else:
            attened_x = self.attns[0](qkv, x.mean(dim=1))
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],
        ...
        attn_types = [(attn_type[0] if attn_type[0] != 'M' else ('I' if i < int(attn_type[4:]) else 'S')) for i in range(depth[0])]
        split_sizes = [(int(la_split_size[0]) if attn_types[i] == 'I' else split_size[0]) for i in range(depth[0])]
        ...
        curr_dim = curr_dim * 2
        attn_types = [(attn_type[1] if attn_type[1] != 'M' else ('I' if i < int(attn_type[4:]) else 'S')) for i in range(depth[1])]
        split_sizes = [(int(la_split_size[1]) if attn_types[i] == 'I' else split_size[1]) for i in range(depth[1])]
        ...
        curr_dim = curr_dim * 2
        attn_types = [(attn_type[2] if attn_type[2] != 'M' else ('I' if i < int(attn_type[4:]) else 'S')) for i in range(depth[2])]
        split_sizes = [(int(la_split_size[2]) if attn_types[i] == 'I' else split_size[2]) for i in range(depth[2])]
        temp_stage3 = []
        temp_stage3.extend(
            [CSWinBlock(
        ...
        curr_dim = curr_dim * 2
        attn_types = [(attn_type[3] if attn_type[3] != 'M' else ('I' if i < int(attn_type[4:]) else 'S')) for i in range(depth[3])]
        split_sizes = [(int(la_split_size[3]) if attn_types[i] == 'I' else split_size[3]) for i in range(depth[3])]
...
def inline_cswin_tiny(pretrained=False, **kwargs):
...
def inline_cswin_small(pretrained=False, **kwargs):
...
def inline_cswin_base(pretrained=False, **kwargs):
...
def inline_cswin_base_384(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[3, 6, 29, 2],
